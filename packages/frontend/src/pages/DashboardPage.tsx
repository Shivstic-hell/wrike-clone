        ...
        <div key={task.id} className="flex items-center gap-3 px-5 py-3">
          <div
            className={clsx('h-2 w-2 rounded-full', {
              ...
              'bg-amber-400': task.status === 'in_review',
              'bg-slate-300': task.status === 'todo' || task.status === 'backlog',
...
export default function DashboardPage() {
  const { data: tasksData, isLoading, error, refetch } = useTasks({ perPage: 100 });

  // Widget calculations
  const totalTasks = tasksData?.data?.length || 0;
  const overdueTasks =
    tasksData?.data?.filter(
      (t: Task) => t.dueDate && new Date(t.dueDate) < new Date() && t.status !== 'done' && t.status !== 'cancelled',
    ).length || 0;
  const inProgressTasks =
    tasksData?.data?.filter((t: Task) => t.status === 'in_progress' || t.status === 'in_review').length || 0;
    ...
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Welcome to your project overview.
        </p>
      </div>
