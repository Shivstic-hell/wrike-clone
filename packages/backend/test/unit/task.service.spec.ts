...
function createQb(): any {
  ...
  const self = qb;
  const methods = [
    'from', 'select', 'where', 'andWhere', 'whereIn', 'whereNull',
    'join', 'leftJoin', 'orderBy', 'limit', 'offset', 'clearSelect',
    'insert', 'update', 'clone', 'count',
  ];
...
describe('TaskService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [TaskService, { provide: DATABASE_PROVIDER, useValue: mockDb }],
    }).compile();
  ...
  describe('findAll', () => {
    it('returns paginated tasks', async () => {
      ...
      qb.offset.mockResolvedValue([{ id: 'task-1', title: 'Task 1' }, { id: 'task-2', title: 'Task 2' }]);
  ...
  describe('create', () => {
    it('creates a task successfully', async () => {
      tenantContext.enterWith({ tenantId: 't1', userId: 'u1', membershipId: 'm1', role: 'admin', permissions: ['*'] });
      qb.first.mockResolvedValueOnce({ id: 'proj-1', tenant_id: 't1' });
      qb.returning.mockResolvedValue([{ id: 'task-new', title: 'New Task', status: 'todo' }]);

      const result = await service.create({ projectId: 'proj-1', title: 'New Task' });
    ...
    it('throws when project missing', async () => {
      tenantContext.enterWith({ tenantId: 't1', userId: 'u1', membershipId: 'm1', role: 'admin', permissions: ['*'] });
      qb.first.mockResolvedValue(null);
      await expect(service.create({ projectId: 'nope', title: 'Task' })).rejects.toThrow('Project not found');
    });
  });
  ...
  describe('update', () => {
    it('updates task with valid changes', async () => {
      tenantContext.enterWith({ tenantId: 't1', userId: 'u1', membershipId: 'm1', role: 'admin', permissions: ['*'] });
      qb.first.mockResolvedValueOnce({ id: 'task-1', title: 'Old', status: 'todo', priority: 'medium', tenant_id: 't1' });
  ...
  describe('bulkUpdate', () => {
    it('updates multiple tasks', async () => {
      tenantContext.enterWith({ tenantId: 't1', userId: 'u1', membershipId: 'm1', role: 'admin', permissions: ['*'] });
      qb.first
        .mockResolvedValueOnce({ id: 'task-1', title: 'A', status: 'todo', tenant_id: 't1' })
        .mockResolvedValueOnce({ id: 'task-2', title: 'B', status: 'todo', tenant_id: 't1' });
      qb.returning
        .mockResolvedValueOnce([{ id: 'task-1', status: 'done' }])
        .mockResolvedValueOnce([{ id: 'task-2', status: 'done' }]);

      const results = await service.bulkUpdate({ taskIds: ['task-1', 'task-2'], updates: { status: 'done' as any } });
  ...
  describe('createDependency', () => {
    it('creates a valid dependency', async () => {
      ...
      qb.returning.mockResolvedValue([{ id: 'dep-1', task_id: 'task-1', depends_on_task_id: 'task-2' }]);

      const result = await service.createDependency({
        taskId: 'task-1', dependsOnTaskId: 'task-2', dependencyType: 'finish_to_start' as any, lagDays: 0,
      });
    ...
    it('rejects self-dependency', async () => {
      ...
      await expect(service.createDependency({
        taskId: 'same-id', dependsOnTaskId: 'same-id', dependencyType: 'finish_to_start' as any, lagDays: 0,
